...
import com.chethan.example.db.DbTest
...
class ProductCategoryDaoTest : DbTest() {
    ...
    fun insertAndRead() {
        ...
        MatcherAssert.assertThat(
            ...
            CoreMatchers.`is`("https://s3-ap-northeast-1.amazonaws.com/m-et/Android/json/all.json")
        )
    }
    ...
    fun createIfNotExists_exists() {
        ...
        MatcherAssert.assertThat(
            db.productCategoryDao().createProductCategoryIfNotExists(item),
            CoreMatchers.`is`(-1L)
        )
    }
    ...
    fun deleteAll() {
        ...
        // check data size after deletion
        val listOfItemsAfterDeletingAll =
            getValue(db.productCategoryDao().loadAllTheProductCategory())
